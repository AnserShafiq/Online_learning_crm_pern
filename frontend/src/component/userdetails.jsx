...
const UserDetails = () => {
  const {user, timeSpent, timer,updateTimeSpent} = useUserStore();
  ...
  useEffect(() => {
    const updateTime = () => {
      updateTimeSpent(timeDisplay)
    }
    window.addEventListener('beforeunload', updateTime)
  })

  return (
    <div className="grid grid-cols-4 border gap-4 flex-wrap w-full bg-dull px-10 py-10">
        {/* User Name */}
        ...
        {/* Time Spent */}
        <div className="inline-flex gap-2">
          <h2 className="font-semibold uppercase text-emerald-500 tracking-wide"><span className="border-b border-emerald-500 ">Time Spent</span> : </h2><h2 className="font-normal capitalize">{Math.floor(timeDisplay / 3600)} hours {Math.floor((timeDisplay%3600) / 60)} min {timeDisplay % 60} sec</h2>
        </div>
    </div>
  )
}
